...
(function () {
    ...
    UI.addClass({
        ...
        return function () {
             ...
             * @example
             * ```js
             * ax5Grid.setWidth(_width);
             * ```
             */
            this.setWidth = function (_width) {
            	if (_width == "100%") {
            		_width = this.$target.offsetParent().innerWidth() - 25;
            	}
            	
            	var _colGroupWidth = 0;
            	this.colGroup.forEach(function (c, ci) {
            		_colGroupWidth = _colGroupWidth + c._width;
                });
            	
            	var _term = (_width - _colGroupWidth) / this.colGroup.length;
            	this.colGroup.forEach(function (c, ci) {
            		c._width = c._width + _term;
                });
...
(function () {
    ...
    var edit_autocomplete = {
            ...
            init: function init(_root, _columnKey, _editor, _$parent, _value) {
                ...
                ajaxPost(eConfig.url, eConfig.param, function (data) {
                	eval("if (data." + eConfig.dataKey + " != null) masterData = data." + eConfig.dataKey + ";");
                	self.bindUI(_root, _columnKey, $el, _editor, _$parent, _value, masterData);
                });
            ...
            bindUI: function bindUI(_root, _columnKey, _$el, _editor, _$parent, _value, _masterData) {
                ...
                _$el.autocomplete({
            		...
            		select: function (event, ui) {
            			var _duplicationResult = false;
            			
            			_root.list.forEach(function (l) {
                			var _a = new Object();
                			var _b = new Object();
                			eConfig.mappingColumn.forEach(function (m) {
                				_a[m] = l[m];
                				_b[m] = ui.item[m];
                            });
                			if(JSON.stringify(_a) == JSON.stringify(_b)) _duplicationResult = true;
                        });
            			
                		if(_duplicationResult) {
                			alert("duplication data");
                			
                			GRID.body.repaint.call(_root, true);
                            GRID.scroller.resize.call(_root);
                		} else {
                			var _selectedColumn = _root.selectedColumn;
                        	for (var c in _selectedColumn) {
                        		var _column = _selectedColumn[c];
                            	if (_column) {
                            		var _dindex = _column.dindex;
                            		var _doindex = _column.doindex;
                            		var _rowIndex = _column.rowIndex;
                            		var _colIndex = _column.rowIndex;
                            		var _item = _root.list[_column.dindex];
                            		
                            		eConfig.mappingColumn.forEach(function (m) {
                            			GRID.data.setValue.call(_root, _dindex, _doindex, m, ui.item[m]);
                            			_item[m] = ui.item[m];
                                    });
                ...
                _$el.on("keyup", function (e) {
                	$("#diagnosisForm input[name=diagCd]").val(_$el.val());
                	var url = "/md/diagnosis_search.do";
                	var param = $('#diagnosisForm').serialize();

                    ajaxPost(url , param, function (data) {
                    	if (data.diagnosis_master_data != null) {
                        	var diagnosisMasterData = data.diagnosis_master_data;
                        	
                        	_$el.autocomplete({
                        		source: diagnosisMasterData,
                        		select: function (event, ui) {
                        			var _selectedColumn = _root.selectedColumn;
                                	for (var c in _selectedColumn) {
                                		var _column = _selectedColumn[c];
                                    	if (_column) {
                                        	...
                                        	_item.diagCd = ui.item.diagCd;
                                        	_item.diagNm = ui.item.diagKrNm;
    ...
    GRID.inlineEditor = {
        ...
        "autocomplete": edit_autocomplete
    };
})();
// ax5.ui.grid.page
(function () {
